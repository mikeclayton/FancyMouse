...
public static class LayoutHelper
{
    ...
    */
    ...
    public static PreviewLayout GetPreviewLayout(
        PreviewStyle previewStyle, DisplayInfo displayInfo, SizeInfo? maximumSize = null, decimal statusBarHeight = 0m)
    {
        ...
        var previewLayout = LayoutHelper.CreateInitialPreviewLayout(previewStyle, previewWindowStyle, previewPaneStyle, maximumSize, statusBarHeight);
    ...
    /// <summary>
    /// Takes the size of a PreviewPane's layout and uses it as the baseline
    ...
    /// </summary>
    public static BoxBounds GetPreviewWindowBounds(SizeInfo previewSize, BoxStyle previewWindowStyle, decimal statusBarHeight = 0m)
    {
        ArgumentNullException.ThrowIfNull(previewSize);
        ArgumentNullException.ThrowIfNull(previewWindowStyle);
        var contentBounds = new RectangleInfo(0, 0, previewSize.Width, previewSize.Height + statusBarHeight);
    ...
    internal static PreviewLayout.Builder CreateInitialPreviewLayout(
        PreviewStyle previewStyle, BoxStyle previewWindowStyle, BoxStyle previewPaneStyle, SizeInfo? maximumSize = null, decimal statusBarHeight = 0m)
    {
        ArgumentNullException.ThrowIfNull(previewStyle);
        ...
        // window's margin/border allowance, and any reserved status bar strip, have been
        // subtracted from it - pure size arithmetic, since PreviewLayout deliberately has no
        // position, only a size (see PreviewLayout). See GetPreviewLayout's statusBarHeight
        // remarks for why the status bar itself isn't part of this size.
        var paneMaxSize = windowMaxSize
            .Shrink(previewWindowStyle.MarginStyle)
            .Shrink(previewWindowStyle.BorderStyle);
        paneMaxSize = new SizeInfo(paneMaxSize.Width, paneMaxSize.Height - statusBarHeight);
